...
filename=strcat(datestr(clock,'yy-mm-dd-HH-MM-SS'),'E_mic_0Gs.txt');
fid=fopen(filename,'wt');       %将V_X_RESULT的数据写入到V_X_RESULT文件中导出
[row,col]=size(E_mic);
for i=1:1:row
  for j=1:1:col
    if (j==col)
...
filename=strcat(datestr(clock,'yy-mm-dd-HH-MM-SS'),'B_mic_0Gs.txt');
fid=fopen(filename,'wt');       %将V_X_RESULT的数据写入到V_X_RESULT文件中导出
[row,col]=size(B_mic);
for i=1:1:row
  for j=1:1:col
